...
# ベクトルを図示する関数（AとQを重ねて表示）
def plot_combined_vectors(A_vectors, Q_vectors, colors_A, colors_Q, title):
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111, projection='3d')
    ax.set_aspect('equal') # アスペクト比を1:1:1に設定

    # ベクトル A の描画
    for i, vector in enumerate(A_vectors):
        x, y, z = vector
        ax.quiver(0, 0, 0, x, y, z, color=colors_A[i], length=1.0, arrow_length_ratio=0.1, label=f'A{i+1}')

    # ベクトル Q の描画
    for i, vector in enumerate(Q_vectors):
        x, y, z = vector
        ax.quiver(0, 0, 0, x, y, z, color=colors_Q[i], length=1.0, arrow_length_ratio=0.1, label=f'Q{i+1}')
    ...
    # 軸の範囲を調整
    combined_vectors = np.concatenate((A_vectors, Q_vectors), axis=0)
    max_val = np.max(np.abs(combined_vectors))
...
# ベクトル A と Q を同じ図に描画
plot_combined_vectors(A_vectors, Q_vectors, colors_A, colors_Q, 'Original Vectors (Columns of A) and Orthonormal Vectors (Columns of Q)')
