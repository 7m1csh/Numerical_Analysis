# Practice 10.08 特異値分解とアニメーション
# 特異値分解を用いて行列の変換を視覚化するアニメーションを作成します。
# 特異値分解は、行列を3つの行列の積に分解する方法です。
# 具体的には、行列 A を次のように分解します：
# A = UΣV^T

print(
"""
行列 A は、「空間を回転・伸縮・再回転」する操作を表します：
...
Step 2： Σを適用
円が楕円にスケーリングされます（特異値による変形）
特異値が 0 に近いほど、潰れていきます

Step 3： U を適用
楕円が再び回転（または反転）されます
最終的に、これが A による変換の結果になります
"""
)
